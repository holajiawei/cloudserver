...
function headObject(request, response, log, callback) {
    ...
    console.log('PREPARING HEAD REQUEST:', { headers, objectKey, bucketName });
    ...
    return dataClient.headObject(objectGetInfo, reqUids, (err, data) => {
        console.log('CLOUD HEAD RESPONSE', { err, data });
        if (err) {
...
function batchDelete(request, response, log, callback) {
    ...
    console.log('BATCH DELETE HEADERS');
    console.log('headers:', headers);
    console.log('ifUnModifiedSince:', ifUnModifiedSince);
    return _getRequestPayload(request, (err, payload) => {
        if (err) {
            return callback(err);
        }
        ...
        console.log('BATCH DELETE PARSED REQUEST');

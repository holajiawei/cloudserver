...
GET /_/backbeat/multiplebackenddata/<bucket name>/<object key>
POST /_/backbeat/batchdelete
*/

function _headObject(request, log, callback) {
    const { headers, objectKey, bucketName } = request;
    const storageLocation = headers['x-scal-storage-class'];
    console.log('HEADOBJECT storageLocation', storageLocation);
    const objectGetInfo =
        dataClient.toObjectGetInfo(objectKey, bucketName, storageLocation);
    if (!objectGetInfo) {
        log.error('error performing head object in multiple backend', {
    ...
    return dataClient.headObject(objectGetInfo, reqUids, callback);
}

function headObject(request, response, log, callback) {
    // const err = _checkMultipleBackendRequest(request, log);
    // if (err) {
    //     return callback(err);
    // }
    return _headObject(request, log, (err, data) => {
        if (err) {
            log.info('head object failed', {
                method: 'headObject',
            });
        }
        const dataRetrievalInfo = {
...
function _ifUnModifiedSince(request, response, log, callback) {
    const { headers } = request;
    return async.waterfall([
        next => _headObject(request, log, next),
        (data, next) => {
            console.log('_ifUnModifiedSince');
            console.log('data:', data);
            console.log('if-unmodified-since:', headers['if-unmodified-since']);
            if (data.LastModified !== headers['if-unmodified-since']) {
                return next(null);
            }
            console.log('HELLO');
            return handleTaggingOperation(
                request, response, 'Put', undefined, log, callback);
        },
    ], err => {
        console.log('FINISHED PUTTING CONDITIONAL TAGS', { err });
        if (err) {
            log.error('setting tags failed', {
                method: '_ifUnModifiedSince',
                error: err,
            });
            return callback(err);
        }
        log.debug('setting tags successful');
...
function batchDelete(request, response, log, callback) {
    ...
    if (ifUnModifiedSince) {
        return _ifUnModifiedSince(request, response, log, callback);
    }
    return _getRequestPayload(request, (err, payload) => {
        if (err) {
            return callback(err);
        ...
        return async.eachLimit(locations, 5, (loc, next) => {
            ...
            // TODO: perform head request. If there is a response with data,
            // get the last modified value from the request.
...
function routeBackbeat(clientIP, request, response, log) {
    ...
    const useMultipleBackend =
        ...
        request.resourceType === 'multiplebackendmetadata';
